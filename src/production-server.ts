...
const CHANNEL_INVITE_LINK = "https://t.me/+oRVEErdg48M1ZTY8";
...
async function checkChannelMembership(
  ...
  try {
    console.log(
      `🔍 [Channel Check] Checking membership for user ${userId} in channel ${REQUIRED_CHANNEL_ID}`,
    );
    ...
    console.log(
      `📊 [Channel Check] User ${userId} status: ${status}, isMember: ${isMember}`,
    );
...
async function setupWebhookAutomatically() {
  if (!WEBHOOK_URL) {
    ...
    console.log(
      "💡 Set TELEGRAM_WEBHOOK_URL or REPLIT_DEV_DOMAIN in environment variables",
    );
    return false;
  }
